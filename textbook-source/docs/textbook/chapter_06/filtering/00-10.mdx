...
Let's drill down a bit. We can add detail that shows each of these assumptions.
```mermaid
flowchart TD
  id0(Ask the user some questions)
  id0 --> id1(For each service in the list)
  id1 --> id2{{Does the service meet the user's circumstances?}}
  id2 -- Yes --> id3(Add the service to the display list)
  id2 -- No --> id4(Do not add the<br>service to the<br>display list)
