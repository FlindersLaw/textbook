...
import {DisplayChapter, DisplayGlossaryItem, DisplayImage} from '@site/src/components';

:::tip COMMENTS
A summary of the changes I've made:

- Any reference to a chapter should not be literal.  Instead use the `<DisplayChapter />` component.  Instructions on using `<DisplayChapter />` is in the code.
- Likewise for glossary items, such as 'code block', 'question block', 'variable' etc.  I have a list of glossary items and when we refer to them, at least initially or the first few times, we should use `<DisplayGlossaryItem />`.  Currently this component doesn't do much but eventually I'm hoping to generate an index from it and/or allow users to click on a term and have a pop-up definition or something.
- Literal terms ie: variable names, values etc should be `displayed as code` and not as plain text.
- I changed 'Boolean' to 'boolean' :-)

The Components are documents (maybe a bit poorly) in `src/components/<ComponentName>/index.js`.  Feel free to build them out or to add more data (eg: glossary items) to them.
:::

In <DisplayChapter chapter='chapter_02' /> , we learned
...
in <DisplayChapter chapter='chapter_02' nourl />,
...
<Components.DisplayImage imageURL='/img/textbook/chapter_03/variable_example1.PNG' />

For example,
in <DisplayChapter chapter='chapter_02' nourl /> we
...
The first datatype - text - is called a <DisplayGlossaryItem item='string' /> in Python. When we refer to strings from now on in this textbook we are referring to text. You will see this term a lot in coding. 
...
:::tip SUGGESTION
Above should read '...for any number which is not a whole number, such as numbers with decimals. Fractions are always converted into `float`s in Python.
:::

By contrast, the third datatype - `integer` - has no decimal points. Integers can only be positive or negative whole numbers.

The final datatype is a new one - <DisplayGlossaryItem item='boolean' />. A <DisplayGlossaryItem item='boolean' /> datatype can only ever be one of two values, true or false. This might seem a bit trivial, but booleans are really important in programming. Many (if not all) of the questions that you present in your app can be reduce to a true or false proposition. We will see this a lot very soon.
...
:::tip SUGGESTION
We should have an admonition for the `=`/`==` confusion to bring more attention to it.
:::
...
1. We can embed logic in our <DisplayGlossaryItem item='questionblock' plural /> using `%` tags. You might recall we did this in <DisplayChapter chapter='chapter_02' nourl />:
...
<DisplayImage imageURL='/img/textbook/chapter_03/codeblock_example1.PNG' />

3. We can embed code into <DisplayGlossaryItem item='mako' upper /> fields. Although you *can* do it, we do not cover it in too much detail in this textbook.
...
:::tip SUGGESTION
Include link to `C2_01_immigration_alert.yml` in this page.  Should we remove the `C2_01` part?  Not as relevant in this topic
:::
...
```yaml showLineNumbers title="C2_02_age_eligibility.yml" showLineNumbers
...
:::tip SUGGESTION
I think splitting the `MJFage` test across two lines might be confusing.  Also, wonder whether we should work into this in more detail?  For example, introduce an example `if ... and ...:` and then move to the `MJFage` example in isolation and then display this code.
:::
...
1. It will find the <DisplayGlossaryItem item='mandatoryblock' /> at line 3 and start processing that.
...
4. It tries to execute that code block, but finds it needs a value for `MJFage`.
...
<Components.DisplayImage imageURL='/img/textbook/chapter_03/show_execution_order1.PNG' />

<Components.DisplayImage imageURL='/img/textbook/chapter_03/show_execution_order2.PNG' />

## Embedded If Statements 2
Let's come back to embedded if statements by reference to another example:

```yaml showLineNumbers title="C2_03_age_eligibility.yml" showLineNumbers
