...
import {Explanation} from '@site/src/components/Quizlet/explanation.js'

<Quiz>
    This is a **Quiz** component.  It's used to display quizlets in the textbook.  The 
    ...
    after opening with the `<Quiz>`.

    #### Title
    You can give your quiz a title by using the `title` keyword ie: `<Quiz title='My Quiz Title'>`.
    ...
    <Answer>
    For short answers it's best not to list the entire component on one line ie:
    `<Answer>Text of answer</Answer>`.  Doing otherwise inserts a line break at the
    end of the answer.  See the two examples below.
    </Answer>
    <Answer>Entry on single line. Looks nicer.</Answer>
    <Answer>
    With line break as the entire text as the `</Answer>` tag appears
    on a separate line.
    </Answer>
