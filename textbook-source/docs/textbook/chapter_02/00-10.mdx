...
It is a tradition in coding, long before this author was a boy, that the first program a programmer should write is
...
1. Go to your [Playground](/docs/textbook/chapter_01/00-30#the-playground)
1. Click on the 'Add' button
1. Call your program `hello_world.yml`
<Components.DisplayImage imageURL='/img/textbook/chapter_02/hello_world1.png' />
1. Enter the code displayed in the code snippet below into the [text editor](/docs/textbook/chapter_01/00-30#3-text-editor)
1. Press <Components.DisplayDAButtons button={Constants.da_save_prop} />

```yaml title="hello_world.yml" showLineNumbers
---
# Our first program!
mandatory: True
...
<Components.DisplayImage imageURL='/img/textbook/chapter_02/code_block1.png' />
</details>

Your text editor should look like this:

<Components.DisplayImage imageURL='/img/textbook/chapter_02/hello_world2.png' altText='Code in text editor' />
...
In Docassemble, we write our code in <Components.DisplayGlossaryItem item='block' />s.  We might use blocks to ask questions, display screens,
...
### Line 2
This is a <Components.DisplayGlossaryItem item='comment' /> line.  Comment lines start with a `#` character.  When Docassemble
...
### Line 3
This is a <Components.DisplayGlossaryItem item='mandatory' /> tag. Adding a mandatory tag and setting it to `True` tells Docassemble that it
must execute this block.  Docassemble does not always execute each block.  We'll to more about block execution
later in this textbook.  For now, just remember that to force a block to run you must set it as
`mandatory: True`.

:::info
We usually refer to a block with `mandatory: True` in it as a <Components.DisplayGlossaryItem item='mandatoryblock' />
:::
...
UPDATE: I have a `DisplayGlossaryItem` component.  Glossary items need to
be stored either in there or in chapter 6 (as originally planned)
:::
## Run the program

Press <Components.DisplayDAButtons button={Constants.da_save_and_run_prop} />. Your program
will run and should display this output:

<Components.DisplayImage altText='Hello World output' imageURL='/img/textbook/chapter_02/hello_world3.png' />
