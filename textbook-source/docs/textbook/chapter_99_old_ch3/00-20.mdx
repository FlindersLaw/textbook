...
import {DisplayChapter, DisplayDAButtons} from '@site/src/components';
import {da_continue_prop, da_restart_prop, da_exit_prop} from '@site/src/constants';
...
<Components.DisplayImage imageURL='/img/textbook/chapter_03/eligible_ineligible_flowchart.PNG' />
...
```yaml showLineNumbers title="C3_01.yml" showLineNumbers
...
# eligibility requirements have been met and if they are directs the user 
# to the 'eligible' endpoint. Otherwise, it directs the user to the 
...
:::info
When developing your own applications in Docassemble, you should use lots of comments
:::
...
Third, we have replaced the <DisplayDAButtons button={da_restart_prop} /> button with a <DisplayDAButtons button={da_continue_prop} /> button.
The Continue button tells Docassemble to move onto the next mandatory block, which in this case is the code block at line 23.
...
:::tip SUGGESTION
Should we discuss (in an info box?) the difference between testing for true/false and testing
for equality of strings.  Because sometimes students confuse testing for `variable == 'Yes'` and 
`variable == True`
:::
...
To trigger our endpoint, we need to match it up with an `event` tag. This event tag will tell a particular code block to run. You can see that we have two `event` tags - one on line 56 and one on line 68. If we meet the condition in the if statement on line 31, our application will execute the `eligible` endpoint by calling that endpoint on line 32. Otherwise, our application will execute the `ineligible` endpoint on line 34. Make sure you run through the application and see how it works. 

:::tip SUGGESTION
Generally we should get rid of the `endpoint` obeject for endpoints.  I did this before I 
fully understood Docassemble and I thought that endpoints had to be objects. They obviously
don't need to be and persisting with this syntax could be confusing.
:::

## Implementing Multiple Question Blocks at an Endpoint

For both the ``eligible`` and ``ineligible`` endpoints, we have implemented a <DisplayDAButtons button={da_restart_prop} /> button, which restarts our application, and an <DisplayDAButtons button={da_exit_prop} /> button, which allows us to quit Docassemble. 
...
```yaml showLineNumbers title="C3_01_multiple_endpoints.yml" showLineNumbers
# Basic demonstration of using multiple endpoints for an interview
---
objects:
  - endpoint: DADict
...
buttons:
  ...
  - "I do not accept": exit
---
# This code drives the interview. It determines whether the initial
# eligibility requirements have been met and if they are, directs the user
# to the eligible endpoint. Otherwise, the code directs the user to the
# 'ineligible endpoint'. For the purposes of this exercise, a user is eligible
# to continue if they are between 18 and 65 and are an Australian citizen.
# Note: This code block must be mandatory.
mandatory: True 
code: |
  if MJFage_ok and MJFis_citizen == 'Yes':
...
question: Thanks!
subquestion: |
  ${MJFhidden_code_variable}
  Thank you for your time.
buttons:
  - Exit: exit
  - Restart: restart
---
# We can use a separate code block to drive the additional questions
# we want to ask. We do this by using a code block and having that
# code block set hidden_code_vbariables as the very last thing it
# does. We list out the other variable names attached to our other
# question blocks and Docassemble will ask each of them in order
code: |
  MJFmain_program_result
  MJFwas_code_useful
  MJFhidden_code_variable = ''
---
```

Now, how does this code work? If we meet the eligibilty criteria, Docassemble, as before, executes the `eligible` endpoint first. To do so, it needs a value for `MJFhidden_code_variable`. It looks for that in the file, and finds it is declared in the code block starting on line 99. However, because we have referred to `MJFmain_program_result` and `MJFwas_code_useful` before `MJFhidden_code_variable`, Docassemble will try to satisfy these variables first. This way, we can control the execution order for our application! Try this code, and you will see that if we are eligible, the information screen is first displayed, and then the survey, and finally the exit screen. Note that `MJFhidden_code_variable` is populated with an empty string, so it is never displayed on the screen. 
