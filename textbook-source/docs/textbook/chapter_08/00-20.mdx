...
In our application, we will use our list to store each of our pleadings. We first need to declare our list of pleadings. We do this on Line 7 of our application. 

<Components.DisplayImage imageURL='/img/textbook/chapter_08/ch08_declare_DAList.PNG' altText='A screenshot of where we have declared our DAList, soc, in our application.'/>

You might have noticed that `soc` is declared a little differently to `endpoint` immediately above it. Specifically:
...
You might have noticed when working with Jinja previously that your user can either download a PDF or a DOCX template. In some cases though, we might not want to allow our user to download a DOCX document. In this case, we can control what content we display to our user using the `valid formats` tag. When our user fills in their details, they will only be able to download the completed template as a PDF:
