...
In the [previous](/docs/textbook/chapter_04/00-30.mdx) [sections](/docs/textbook/chapter_04/00-40.mdx), we developed a series of applications that used if statements to direct the flow of control for our application. Depending on what the user entered as input, the if statement would trigger an event and Docassemble would display an end screen.
