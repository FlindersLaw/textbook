...
We can also test other values using questions. For example, let's say that we want to develop an app to advise someone on a penalty they may face. The penalty will be greater if this is their first offence. We could use the following notation.

**IF** *this is user's first offence* **THEN** *they receive this penalty* **ELSE** *they receive this penalty*
