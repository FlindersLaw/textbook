...
In the previous [section](./00-30.mdx), we mentioned that there are two ways we can create an if block with multiple conditions:
...
In addition to the [rules](./00-20.mdx) about if statements we discussed previously:
...
### Flow of Control
We could modify our flowchart from the previous [section](./00-30.mdx) to include the following questions:
...
Another example lies in the AusStudy branch of our application. An eagle eyed reader, upon examining the link for AusStudy, might note that this is available for students who are over 25. To help make a condition for this, we will need to use [and and or statements](./00-60.mdx).
