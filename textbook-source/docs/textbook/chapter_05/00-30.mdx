...
Second, imagine if we had 1,000 bread types.  The inconvenience of copying the
same line four times in a five-element list becomes a huge effort for a 1,000-element
list.  Not to mention the possibility of introducing errors.

Third, what happens if we decide to add a new item to the list?  Or what if we
had to remove an item from the list?  Every change we made to the list data
(in the code block) would have to be duplicated in the mandatory question block.
Again, this is inefficient and it's very error prone.  The history of coding
is littered with errors arising from unnecessary copying and pasting of code.

What we really want to do is, instead telling Docassemble to *display the first item*,
then *display the second item*, then *display the third item* and so-on, we
want to tell Docassemble this:
...
Still with us?  Let's re-phrase it in a more code-y way:
...
treats indetation of four spaces or more as a special command.
